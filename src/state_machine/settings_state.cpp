...
bool SettingsState::handleEvent(sf::Event const& event)
{
  ...
  for (std::size_t action = 0;
       action < toUnderlyingType(Player::Action::ACTION_COUNT);
       ++action)
  {
...
void SettingsState::updateLabels() noexcept
{
  ...
  {
    sf::Keyboard::Key key =
        player.getAssignedKey(static_cast<Player::Action>(action));
    m_binding_labels[action]->setText(SFML::toString(key));
  }
}

void SettingsState::addButtonLabel(
    Player::Action action, float y, std::string text, Context context) noexcept
{
  m_binding_buttons[toUnderlyingType(action)] =
  ...
  m_binding_labels[toUnderlyingType(action)] =
