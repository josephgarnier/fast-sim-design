...
namespace FastSimDesign {
  ...
  BloomEffet::BloomEffet() noexcept
    : Parent{}
  {
    m_shaders.load(Shaders::ID::ADD_PASS, "../assets/shaders/fullpass.vert", "../assets/shaders/add.frag");
    m_shaders.load(Shaders::ID::BRIGHTNESS_PASS, "../assets/shaders/fullpass.vert", "../assets/shaders/brightness.frag");
    m_shaders.load(Shaders::ID::DOWN_SAMPLE_PASS, "../assets/shaders/fullpass.vert", "../assets/shaders/down_sample.frag");
    m_shaders.load(Shaders::ID::GAUSSIAN_BLUR_PASS, "../assets/shaders/fullpass.vert", "../assets/shaders/guassian_blur.frag");
  }
  ...
  void BloomEffet::apply(sf::RenderTexture const& input, sf::RenderTarget& output)
  {
    ...
    add(m_first_pass_textures[0], m_second_pass_textures[0], m_first_pass_textures[1]);
  ...
  void BloomEffet::filterBright(sf::RenderTexture const& input, sf::RenderTexture& output)
  {
  ...
  void BloomEffet::blurMultipass(RenderTextureArray& render_textures)
  {
    ...
    {
      blur(render_textures[0], render_textures[1], sf::Vector2f{0.f, 1.f / static_cast<float>(texture_size.y)});
      blur(render_textures[1], render_textures[0], sf::Vector2f{1.f / static_cast<float>(texture_size.x), 0.f});
    }
  }

  void BloomEffet::blur(sf::RenderTexture const& input, sf::RenderTexture& output, sf::Vector2f const& offset_factor)
  {
  ...
  void BloomEffet::downsample(sf::RenderTexture const& input, sf::RenderTexture& output)
  {
  ...
  void BloomEffet::add(sf::RenderTexture const& source, sf::RenderTexture const& bloom, sf::RenderTarget& target)
  {
