...
namespace FastSimDesign {
  ...
  void SceneNode::checkSceneCollision(SceneNode& scene_graph, std::set<SceneNode::Pair>& collision_pairs) noexcept
  {
  ...
  void SceneNode::checkNodeCollision(SceneNode& node, std::set<SceneNode::Pair>& collision_pairs) noexcept
  {
  ...
  void SceneNode::removeWrecks() noexcept
  {
    ...
    auto erase_begin = std::remove_if(std::begin(m_children), std::end(m_children), [](Ptr const& child) {
      return child->isMarkedForRemoval();
    });
    m_children.erase(erase_begin, std::end(m_children));

    // Call function recursively for all remaining children.
    std::for_each(std::begin(m_children), std::end(m_children), [](Ptr const& child) {
      child->removeWrecks();
    });
  }
