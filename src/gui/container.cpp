...
{
  // If we have selected a child then give it events ; the active component is
  // the one that should receive the events instead of the container managing
  // it.
  if (hasSelection() &&
      m_children[static_cast<std::size_t>(m_selected_child)]->isActive())
  {
...
void Container::selectPrevious() noexcept
{
  ...
  do
  {
    prev =
        ...
        m_children
            .size(); // We need "+ length" to avoid negative indexes, due to the
                     // way modulus works in C++ (it use truncated division).
