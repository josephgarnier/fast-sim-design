...
inline constexpr typename std::
    enable_if<std::is_floating_point<T>::value, sf::Vector2<T>>::type
    normalize(sf::Vector2<T> const& vector)
{
  T length_squared = glm::length2(glm::vec<2, T>{vector.x, vector.y});

  if (length_squared > T{0})
  {
    glm::vec<2, T> unit_vector =
        glm::normalize(glm::vec<2, T>{vector.x, vector.y});
