...
///
/// @details
/// The entry point for Fast-Sim-Design simulations is the Application class and
/// {@link #launch(Application)} method. It initializes application instance and
/// starts the main application loop.
///
/// @section Life-cycle
/// <ol>
///     <li>{@link #configure(Configuration)} It's called before simulation
///     initialization, so only basic application setups are expected.</li>
///     <li>{@link #initWindow(Configuration)} Method creates and initialize
///     SFML window.</li> <li>{@link #initImGui(Configuration)} Method
///     initializes Dear ImGui context. Could be used to do Dear ImGui setup as
///     well.</li> <li>{@link #preRun()} Method called once, before simulation
///     loop.</li> <li>{@link #beginFrame()} Method called at the beginning of
///     every frame, before {@link #processInput()}.</li> <li>{@link
///     #processInput()} Method called every frame, before {@link #process()},
///     to process user inputs.</li> <li>{@link #update()} Method called every
///     frame to update simulation state and Dear ImGui states from input.</li>
///     <li>{@link #updateStatistic()} Method called every frame to update basic
///     simulation statistic.</li> <li>{@link #render()} Method called every
///     frame to render simulation state and Dear ImGui states.</li> <li>{@link
///     #endFrame()} Method called at the end of every frame, after {@link
///     #render()}.</li> <li>{@link #postRun()} Method called once, after
///     simulation loop.</li> <li>{@link #disposeImGui()} Destroys Dear ImGui
///     context.</li> <li>{@link #disposeWindow()} Destroys SFML window.</li>
/// </ol>
///
/// @section Reference
/// This class in inspired from \link
/// https://github.com/SpaiR/imgui-java/blob/main/imgui-app/src/main/java/imgui/app/Application.java
/// \endlink and \link
...
protected:
  /// Method called before simulation initialization. Could be used to provide
  /// basic window information in editing `config`, like title name, main window
  /// size, etc.
  ...
  /// Method called every frame to render simulation state and Dear ImGui
  /// states.
  void render();
...
private:
  ...
  std::size_t m_statistics_num_frames{0};
  std::size_t m_statistics_total_frames{0};
  std::size_t m_statistics_total_update_frames{0};
  std::size_t m_statistics_total_render_frames{0};
};
} // namespace FastSimDesign
#endif
