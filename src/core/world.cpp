...
namespace FastSimDesign {
  ...
  void World::spawnEnemies() noexcept
  {
    ...
    while (!m_enemy_spawn_points.empty() && m_enemy_spawn_points.back().y > getBattlefieldBounds().top)
    {
  ...
  {
    return sf::FloatRect(m_world_view.getCenter() - m_world_view.getSize() / 2.f, m_world_view.getSize());
  }
