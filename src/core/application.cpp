...
Application::Application()
  ...
  , m_state_stack{
        State::Context{&m_monitor, &m_window, &m_textures, &m_fonts, &m_player}}
{
  ...
  m_textures.load(
      Textures::ID::TITLE_SCREEN,
      "../assets/sprites/npcs/title_screen.png");
...
void Application::initMonitor(Configuration const& config) noexcept
{
  ...
  m_monitor
      .getWindow<SimMonitor::StateMachineWindow>(
          SimMonitor::Window::ID::STATE_MACHINE)
      .setDataModel(&m_state_stack);
}
...
{
  LOG_INFO(
      "*** Run app with a tick timer at {}ms per frame ***",
      Application::TIME_PER_FRAME.asMilliseconds());
...
void Application::runFrame(
    sf::Clock& loop_clock, sf::Time& time_since_last_update)
{
  sf::Time delta_time = loop_clock.restart();
  beginFrame();
  LOG_TRACE(
      "*** Previous frame duration is {}ms ***",
      elapsed_time.asMilliseconds());
  ...
  while (time_since_last_update > Application::TIME_PER_FRAME &&
         nb_updates < Application::MAX_UPDATES)
  {
...
void Application::updateStatistics(sf::Time const& dt) noexcept
{
  ...
  if (m_statistics_update_time >= sf::seconds(1.0f))
  {
    m_statistics_text.setString(
        "Sim Time = " +
        std::to_string(m_statistics_sim_time.getElapsedTime().asSeconds()) +
        "s" + "\n" +
        "Frames / Second (FPS) = " + std::to_string(m_statistics_num_frames) +
        "\n" + "Time / Update (TPF) = " +
        std::to_string(
            static_cast<size_t>(m_statistics_update_time.asMicroseconds()) /
            m_statistics_num_frames) +
        "us" + "\n" + // Time per frame
        "Nb Frames = " + std::to_string(m_statistics_total_frames) + "\n" +
        "Nb Update Frame = " +
        std::to_string(m_statistics_total_update_frames) + "\n" +
        "Nb Render Frame = " +
        std::to_string(m_statistics_total_render_frames));
