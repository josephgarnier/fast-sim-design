...
namespace FastSimeDesign {
  ...
  std::array<fmt::text_style, spdlog::level::n_levels> LogFormater::m_colors = []() {
    ...
    colors[spdlog::level::critical] = fmt::fg(fmt::terminal_color::bright_red);
  ...
  /// Methods
  ////////////////////////////////////////////////////////////
  void LogFormater::format(spdlog::details::log_msg const& msg, spdlog::memory_buf_t& dest)
  {
    ...
    fmt::format_to(std::back_inserter(msg_source_buffer), "{}", msg.source.filename ? basename(msg.source.filename) : "unknown_file");
    msg_source_buffer.push_back(':');
    msg_source_buffer.push_back(':');
    fmt::format_to(std::back_inserter(msg_source_buffer), "{}", msg.source.funcname ? msg.source.funcname : "unknown_function");
    msg_source_buffer.push_back('(');
    msg_source_buffer.push_back(')');
    msg_source_buffer.push_back(']');
    fmt::format_to(std::back_inserter(msg_buffer), "{:<37}", fmt::to_string(msg_source_buffer));

    // Add %v.
    spdlog::details::fmt_helper::append_string_view(msg.payload, msg_buffer);

    // Add style color depending of the log level.
    fmt::format_to(std::back_inserter(dest), msg_style, "{}\n", fmt::to_string(msg_buffer));
  }
