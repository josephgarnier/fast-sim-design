...
/// This function is inspired from \link
/// https://gist.github.com/Lima-X/73f2bbf9ac03818ab8ef42ab15d09935 \endlink and
/// from \link
/// https://github.com/ocornut/imgui/blob/6982ce43f5b143c5dce5fab0ce07dd4867b705ae/imgui_demo.cpp#L8683
/// \endlink.
...
void LogWindow::backupFilterState()
{
  ...
  memcpy(
      m_previous_filter_state.text_filter,
      m_text_filter.InputBuf,
      sizeof(m_previous_filter_state));
}

void LogWindow::drawButton(
    char const* label, std::function<void()> const& action) const
{
...
void LogWindow::drawLogFilter()
{
  ...
  [[maybe_unused]] ImVec2 outer_size = ImVec2{
      0.0f,
      window_size.y - (ImGui::GetFontSize() + ImGui::GetFrameHeight() * 2)};
  ...
  static auto const log_selection_width = []() -> float {
    ...
    return longest_text_width + ImGui::GetStyle().FramePadding.x * 2.f +
           ImGui::GetFrameHeight();
  }();

  auto combo_box_right_alignment =
      window_size.x - (log_selection_width + ImGui::GetStyle().WindowPadding.x);
  ImGui::Text("Filter:");
  ImGui::SameLine();
  m_text_filter.Draw(
      "##TextFilter",
      -(log_selection_width + ImGui::GetStyle().WindowPadding.x * 2.f +
        ImGui::GetStyle().FramePadding.x));
  ImGui::SetNextItemWidth(log_selection_width);
  ImGui::SameLine(combo_box_right_alignment);
  ImGui::Combo(
      "##LevelFilter",
      toUnderlyingType<spdlog::level::level_enum>(&m_log_level_filter),
      log_levels,
...
bool LogWindow::isFilterModified() const
{
  ...
  filters_modified |=
      memcmp(p_text_filter, m_text_filter.InputBuf, sizeof(p_text_filter));
...
bool LogWindow::doesPassFilter(
    std::pair<int, LogData const&> log_metadata) const
{
  auto isPassLevelFilter = [this](spdlog::level::level_enum level) {
    return m_log_level_filter == spdlog::level::off ||
           level == m_log_level_filter;
  };
  auto isPassTextFilter = [this](char const* line_start, char const* line_end) {
    return !m_text_filter.IsActive() ||
           m_text_filter.PassFilter(line_start, line_end);
  };
  ...
  char const* line_end = (log_index + 1 < m_log_metadata.Size)
                             ? (m_log_text.begin() +
                                m_log_metadata[log_index + 1].m_line_offset - 1)
                             : m_log_text.end();
...
void LogWindow::drawLogLines() const
{
  ...
  while (clipper.Step())
  {
    for (int clipper_line_number = clipper.DisplayStart;
         clipper_line_number < clipper.DisplayEnd;
         ++clipper_line_number)
    {
      ...
      char const* line_end =
          (log_index + 1 < m_log_metadata.Size)
              ? (buf + m_log_metadata[log_index + 1].m_line_offset - 1)
              : buf_end;
...
void LogWindow::addLog(LogMessage message)
{
  ...
  m_log_text.append(
      ...
      message.m_message.c_str() + message.m_message.size());
  m_log_metadata.push_back({message.m_level, old_size});
  if (doesPassFilter(
          std::make_pair(m_log_metadata.size() - 1, m_log_metadata.back())))
