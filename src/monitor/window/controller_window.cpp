    ...
    void ControllerWindow::updateMenuBar(sf::Time const&)
    {
      if (ImGui::BeginMenuBar())
      {
        ...
        if (ImGui::BeginMenu("View"))
        {
          ...
          if (ImGui::MenuItem("Show Log Window", "Ctrl+L", m_monitor->getWindow<LogWindow>(Window::ID::LOG).isVisible()))
            m_monitor->getWindow<LogWindow>(Window::ID::LOG).switchVisibility();
          if (ImGui::MenuItem("Show State Machine Window", "Ctrl+M", m_monitor->getWindow<StateMachineWindow>(Window::ID::STATE_MACHINE).isVisible()))
            m_monitor->getWindow<StateMachineWindow>(Window::ID::STATE_MACHINE).switchVisibility();
          if (ImGui::MenuItem("Show Scene Graph Window ", "Ctrl+G", m_monitor->getWindow<SceneGraphWindow>(Window::ID::SCENE_GRAPH).isVisible()))
            m_monitor->getWindow<SceneGraphWindow>(Window::ID::SCENE_GRAPH).switchVisibility();
          ImGui::EndMenu();
        }
