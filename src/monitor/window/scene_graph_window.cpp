...
{
  static ImGuiTreeNodeFlags tree_flags = ImGuiTreeNodeFlags_SpanAvailWidth |
...
void SceneGraphWindow::drawTreeSceneNode(
    Frame::SceneNode const& frame_scene_node, uintmax_t& id) const
{
  ...
  if (ImGui::TreeNodeEx(
          ImGuiUtil::labelize(frame_scene_node.category, id).c_str(),
          node_flags,
          "%s:%s (%lld)",
          frame_scene_node.type.c_str(),
          frame_scene_node.category.c_str(),
          id))
  {
